...
namespace ads {
    ...
    class list {
        ...
        public:
            ...
            list& remove_next (_node<T>*);
            _node<T> *find (T refer, int *iter);
            T pop (int index);
            T popStart();
            T popBack();
            int len() const;
    ...
    template <typename T>
    const _node<T> * _node<T>::step_next() const {
        return next;
    }
    ...
    template <typename T>
    list<T>& list<T>::remove_next (_node<T>* refer) {
        _node<T> *next = refer->next->next;
        if (refer->next == end)
            end = refer;
        delete refer->next;
        refer->next = next;
    ...
    template <typename T>
    _node<T>* list<T>::find (T refer, int *iter) {
        _node<T> *cur = &node;
        (*iter) = 0;
        while (cur->next) {
            (*iter)++;
            if (refer == cur->next->field)
                return cur;
            cur = cur->next;
        }
        return nullptr;
